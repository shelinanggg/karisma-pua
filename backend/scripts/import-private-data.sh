...
for sql_file in "$PRIVATE_DATA_DIR"/*.sql; do
  if grep -Eq 'SELECT[[:space:]]+1::integer[[:space:]]+AS[[:space:]]+id_periode_skp' "$sql_file"; then
    echo >&2
    echo "Import dibatalkan: $(basename "$sql_file") memakai id_periode_skp = 1 secara hard-coded." >&2
    echo "Gunakan lookup periode yang eksplisit atau keluarkan file tersebut dari folder private-data." >&2
    exit 1
  fi
